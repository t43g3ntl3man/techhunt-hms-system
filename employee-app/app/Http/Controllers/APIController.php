        ...
        if ($employee = Employee::whereEmail(request('email'))->first()) {
            ...
            // if (Hash::check($request->pin_code, $employee->pin_code)) {
                if (!Attendance::whereAttendance_date(date("Y-m-d"))->whereEmp_id($employee->id)->first()) {
                    ...
                    $attendance_time_str = date("h:i:s A");
                    $attendance->attendance_date = date("Y-m-d");
                    $entry = strtotime('12:00:00 am');
                    $diff = strtotime($attendance_time_str) - $entry;
                    // dd($diff); 
                    if($diff < -100){
                        return response()->json(['error' => 'unnable to mark attendance right now'], 404);
                    } else {
                        if($diff > 10){
                            $attendance->late = 1;
                        } else {
